...
const publishedResults = {
  ...
  environmentalNetFlows: [0.2154, 0.5065, -0.472, -0.2498],
  meanAbsoluteDeviations: [0.3874, 0.2474, 0.3119, 0.323],
};

function calculateGroupNetFlows(firstCriterionIndex: number, endCriterionIndex: number): number[] {
  const groupProblem = new PrometheeDecisionProblem();
  const groupWeights = problem.weights.slice(firstCriterionIndex, endCriterionIndex);
  const groupWeightSum = groupWeights.reduce((sum, weight) => sum + weight, 0);

  groupProblem.alternatives = problem.alternatives;
  groupProblem.criteria = problem.criteria.slice(firstCriterionIndex, endCriterionIndex);
  groupProblem.matrix = problem.matrix.map((alternativeValues) =>
    alternativeValues.slice(firstCriterionIndex, endCriterionIndex),
  );
  groupProblem.weights = groupWeights.map((weight) => weight / groupWeightSum);
  groupProblem.types = problem.types.slice(firstCriterionIndex, endCriterionIndex);
  ...
  const debugResults = groupProblem.debugBag as PrometheeDebugResults;
...
const meanAbsoluteDeviations = problem.alternatives.map((_, alternativeIndex) => {
  ...
  const sumOfAbsoluteDifferences = groupNetFlows.reduce(
    (sum, groupNetFlow) => sum + Math.abs(groupNetFlow - globalNetFlow),
    0,
  );
...
printVectorComparison("Ranks", problem.alternatives, ranks, publishedResults.ranks, 0);
printVectorComparison(
  "Economic net flows",
  problem.alternatives,
  economicNetFlows,
