...
import { CoprasDecisionProblem, CriterionType, sumNormalizationCallback } from "../../../src";
import { rank } from "../../../src/utils/ranking";

// Expected values from konkurencja/python/pyrepo_mcda_check-copras.py (pyrepo_mcda 0.1.8).
const rounded = (values: number[]) => values.map((x) => Math.round(x * 10000) / 10000);
function createProblem(second = false): CoprasDecisionProblem {
  const problem = new CoprasDecisionProblem();
...
describe("CoprasDecisionProblem", () => {
  it("matches the Python example to four decimal places", () => {
    expect(rounded(createProblem().scores)).toEqual([0.912, 1.0, 0.8815]);
  });
  it("matches the four-criterion Python example", () => {
    expect(rounded(createProblem(true).scores)).toEqual([1.0, 0.8184, 0.9459, 0.8176]);
  });
  ...
  it("normalizes costs without reversing them and rejects absent costs", () => {
    const problem = createProblem(true);
    problem.normalizationCallback = (matrix, types) => {
      expect(types).toEqual([1, 1, 1, 1]);
      return sumNormalizationCallback(matrix, types);
    };
    problem.scores;
    problem.types = [1, 1, 1, 1];
    expect(() => problem.scores).toThrow(/divisor/);
  });
});

it("copras default ranking matches the unrounded Python preferences", () => {
  expect(rank(createProblem(false).scores, true)).toEqual([2, 1, 3]);
});

it("copras second ranking matches the unrounded Python preferences", () => {
