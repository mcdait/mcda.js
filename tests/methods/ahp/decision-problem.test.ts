...
// Expected values from konkurencja/python/pyrepo_mcda_check-ahp.py (pyrepo_mcda 0.1.8).
...
it("ahp default ranking matches the unrounded Python preferences", () => {
  expect(rank(createProblem(false).scores, true)).toEqual([2, 1, 3]);
});

it("ahp second ranking matches the unrounded Python preferences", () => {
