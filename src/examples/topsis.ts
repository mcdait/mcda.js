...
const pdfResults = {
    ...
    distanceToBest: [0.0545, 0.1197, 0.0580, 0.1009],
    distanceToWorst: [0.0983, 0.0439, 0.0920, 0.0458],
...
printVectorComparison(
    "Ideal best",
    problem.criteria,
    debugResults.idealBest,
    pdfResults.idealBest,
);
printVectorComparison(
    "Ideal worst",
    problem.criteria,
...
console.table(
    ranking.map((result) => {
        const alternativeIndex = problem.alternatives.indexOf(
            result.alternative,
        );
        ...
        return {
            ...
            rankDifference:
                result.rank - (pdfResults.ranks[alternativeIndex] ?? 0),
        };
    }),
);
console.log(
    `Ranking: ${ranking.map(({ alternative }) => alternative).join(" > ")}`,
);
