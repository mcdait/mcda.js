    ...
    rowNames.map((rowName, rowIndex) => {
      ...
      const differences = columnNames
        .map((columnName, columnIndex) => {
          ...
          return difference === "-" ? undefined : `${columnName}: ${difference}`;
        })
        .filter((difference) => difference !== undefined);

      return {
        [rowNameLabel]: rowName,
        calculated: calculatedRow.map((value) => value.toFixed(decimalPlaces)).join(", "),
        pdf: publishedRow.map((value) => value.toFixed(decimalPlaces)).join(", "),
        differences: differences.length === 0 ? "-" : differences.join(", "),
      };
    }),
  );
}
...
export function printVectorComparison(
  ...
  console.table(
    names.map((name, index) => ({
      ...
      difference: formatDifference(calculated[index] ?? 0, published[index] ?? 0, decimalPlaces),
    })),
  );
}
